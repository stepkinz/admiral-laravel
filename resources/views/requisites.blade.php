      ...
      <section class="py-16 md:py-24 relative bg-white border-b border-slate-200 swiss-pattern-diagonal">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
          <div class="max-w-4xl mx-auto">
            ...
            <h2 class="text-3xl md:text-4xl font-bold tracking-tight font-serif mb-8" style="color: #243468;">Контактные
              данные</h2>
              ...
              <dl class="divide-y divide-slate-200">
                @if($phone)
                  <div class="flex flex-col sm:flex-row sm:py-4 py-3 px-6 gap-2 sm:gap-4 bg-stone-50/50 sm:even:bg-white">
                    <dt class="flex-shrink-0 w-full sm:w-40 text-sm font-medium uppercase tracking-wider text-slate-500">
                      Телефон</dt>
                    <dd class="text-slate-900 font-medium"><a href="tel:{{ preg_replace('/\D/', '', $phone) }}"
                        class="hover:opacity-80" style="color: #243468;">{{ $phone }}</a></dd>
                  </div>
                @endif
                @if($email)
                  <div class="flex flex-col sm:flex-row sm:py-4 py-3 px-6 gap-2 sm:gap-4 bg-white sm:even:bg-stone-50/50">
                    <dt class="flex-shrink-0 w-full sm:w-40 text-sm font-medium uppercase tracking-wider text-slate-500">
                      Email</dt>
                    <dd class="text-slate-900 font-medium"><a href="mailto:{{ $email }}" class="hover:opacity-80"
                        style="color: #243468;">{{ $email }}</a></dd>
                  </div>
                @endif
                @if($address)
                  <div class="flex flex-col sm:flex-row sm:py-4 py-3 px-6 gap-2 sm:gap-4 bg-stone-50/50 sm:even:bg-white">
      ...
      <section class="py-16 md:py-24 relative bg-stone-50 border-b border-slate-200 swiss-pattern-lines">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
          <div class="max-w-4xl mx-auto">
            ...
            <h2 class="text-3xl md:text-4xl font-bold tracking-tight font-serif mb-8" style="color: #243468;">
              ...
              <dl class="divide-y divide-slate-200">
                @if($orgForm)
                  <div class="flex flex-col sm:flex-row sm:py-4 py-3 px-6 gap-2 sm:gap-4">
                    <dt class="flex-shrink-0 w-full sm:w-48 text-sm font-medium uppercase tracking-wider text-slate-500">
                      Организационно-правовая форма</dt>
      ...
      <section class="py-16 md:py-24 relative bg-white border-b border-slate-200 swiss-pattern-diagonal">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
          <div class="max-w-4xl mx-auto">
            ...
            <h2 class="text-3xl md:text-4xl font-bold tracking-tight font-serif mb-8" style="color: #243468;">Банковские
              реквизиты</h2>
            <div class="border border-slate-200 rounded-none overflow-hidden">
              <dl class="divide-y divide-slate-200">
                @if($bankName)
                ...
                @if($corrAccount)
                  <div class="flex flex-col sm:flex-row sm:py-4 py-3 px-6 gap-2 sm:gap-4 bg-white sm:even:bg-stone-50/50">
                    <dt class="flex-shrink-0 w-full sm:w-48 text-sm font-medium uppercase tracking-wider text-slate-500">
                      Корреспондентский счёт</dt>
                    <dd class="text-slate-900 font-mono break-all">{{ $corrAccount }}</dd>
                  </div>
                @endif
              </dl>
              @if(!$bankName && !$bik && !$checkingAccount && !$corrAccount)
                <p class="px-6 py-8 text-slate-500">Банковские реквизиты заполняются в разделе «Реквизиты» админ-панели.
                </p>
              @endif
            </div>
          </div>
      ...
      <section class="py-20 text-slate-900 relative bg-stone-50 border-b border-slate-200 swiss-pattern-lines">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
          <div class="max-w-3xl mx-auto text-center">
            <h2 class="text-3xl md:text-4xl font-bold mb-6 tracking-tight font-serif" style="color: #243468;">Остались
              вопросы?</h2>
            <p class="text-lg text-slate-700 mb-8">Свяжитесь с нами для уточнения реквизитов или условий сотрудничества
            </p>
            <div class="flex flex-col sm:flex-row justify-center gap-4">
              <a href="{{ route('contact') }}"
                class="inline-flex items-center justify-center px-8 py-6 text-base font-bold text-white rounded-none uppercase tracking-wider transition-opacity hover:opacity-90"
                style="background-color: #ED3200;">Связаться с нами</a>
              @if($phone)
                <a href="tel:{{ preg_replace('/\D/', '', $phone) }}"
                  class="inline-flex items-center justify-center px-8 py-6 text-base font-bold border-2 border-slate-900 text-slate-900 rounded-none uppercase tracking-wider hover:bg-slate-50 transition-colors">{{ $phone }}</a>
              @endif
            </div>
          </div>
        </div>
