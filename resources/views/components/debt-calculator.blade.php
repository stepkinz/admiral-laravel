...
<section class="py-16 md:py-24 relative bg-white border-b border-slate-200 swiss-pattern-diagonal" x-data="debtCalculator()" x-init="init()">
  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
    <div class="text-center mb-8 relative">
      ...
      <div class="inline-flex items-center gap-3 bg-slate-100 rounded-none border border-slate-200 p-1">
        <button
          type="button"
          x-on:click="showComparison = false"
          :class="!showComparison ? 'bg-white text-slate-900 border border-slate-200' : 'text-slate-600 hover:text-slate-900'"
          class="px-6 py-2 rounded-none font-medium transition-all"
        >
          💰 С нами
        </button>
        <button
        ...
        <div>
          ...
          <div class="relative mb-4">
            <input
              ...
              class="debt-slider w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
              :style="'background: linear-gradient(to right, #ed3200 0%, #ed3200 ' + (debtAmount / 3000000 * 100) + '%, #e5e7eb ' + (debtAmount / 3000000 * 100) + '%, #e5e7eb 100%)'"
            />
        ...
        <div>
          ...
          <div class="relative mb-4">
            <input
              ...
              class="months-slider w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
              :style="'background: linear-gradient(to right, #ed3200 0%, #ed3200 ' + (12 === minMonths ? 100 : ((months - minMonths) / (12 - minMonths) * 100)) + '%, #e5e7eb ' + (12 === minMonths ? 100 : ((months - minMonths) / (12 - minMonths) * 100)) + '%, #e5e7eb 100%)'"
            />
            <div class="flex justify-between text-xs text-gray-500 mt-2">
          ...
          <div class="space-y-6">
            <div class="bg-gradient-to-br from-green-500 to-green-600 text-white p-6 rounded-xl shadow-lg">
              <div class="flex items-center justify-between mb-3">
                <h3 class="text-lg font-bold">💰 Ваша экономия</h3>
                <span class="text-3xl font-bold" x-text="savingsPercent + '%'"></span>
              </div>
              <div class="bg-white/20 rounded-lg p-3 mb-3">
                <p class="text-sm opacity-90 mb-1">Вы экономите:</p>
                <p class="text-3xl font-bold" x-text="formatNumber(discount) + ' ₽'"></p>
              </div>
              <div class="flex gap-2 text-xs">
                <div class="flex-1 bg-white/20 rounded p-2 text-center">
                  <p class="opacity-75">Было</p>
                  <p class="font-bold" x-text="formatNumber(debtAmount) + ' ₽'"></p>
                </div>
                <div class="flex items-center">→</div>
                <div class="flex-1 bg-white/30 rounded p-2 text-center">
                  <p class="opacity-75">Стало</p>
                  <p class="font-bold" x-text="formatNumber(finalAmount) + ' ₽'"></p>
                </div>
              </div>
            </div>
            <div class="bg-[#8B2D1C] text-white p-6 rounded-xl shadow-lg">
              <h3 class="text-xl font-bold mb-3">Срок погашения:</h3>
              <p class="text-sm mb-4">Сколько времени потребуется для погашения?</p>
              <p class="text-2xl font-bold mb-2">
                <span x-text="months"></span> <span x-text="months === 1 ? 'месяц' : (months < 5 ? 'месяца' : 'месяцев')"></span>, если регулярно вносить по <span x-text="formatNumber(monthlyPayment)"></span> рублей.
              </p>
              <p class="text-sm">
                Последний платеж<br><strong x-text="formatCurrency(lastPayment) + ' рублей.'"></strong>
              </p>
              <div class="mt-4 pt-4 border-t border-white/20">
                <p class="text-sm">ПКО Адмирал не начисляет проценты на остаток задолженности.</p>
              </div>
              <div class="mt-4">
                <p class="text-xs opacity-75 mb-2">Путь к свободе от долгов:</p>
                ...
                <p class="text-xs opacity-75 mt-1">Осталось <span x-text="months - 1"></span> платежей после первого</p>
              </div>
            </div>
            <div class="bg-white border-2 border-gray-200 p-6 rounded-xl shadow-sm">
              <h3 class="text-xl font-bold text-gray-900 mb-3">Сумма списания:</h3>
              <p class="text-sm text-gray-600 mb-4">Сколько получится списать?</p>
              <p class="text-2xl font-bold text-[#ed3200] mb-2">
                <span x-text="formatCurrency(discount)"></span> рублей при заключении и соблюдении платежного соглашения.
              </p>
            </div>
            <button
              type="button"
    ...
    <div class="mt-12 max-w-5xl mx-auto">
      <div class="bg-red-50 border-2 border-red-200 rounded-xl p-6 md:p-8">
        <h3 class="text-2xl font-bold text-red-900 mb-6 text-center">
          ⚠️ Если не воспользоваться нашими услугами
        </h3>
        <div class="grid md:grid-cols-3 gap-4 text-center">
          <div class="bg-white rounded-lg p-4 shadow-sm">
            <div class="text-3xl font-bold text-red-600 mb-2" x-text="'+' + formatNumber(debtAmount * 0.3) + '₽'"></div>
            <p class="text-sm text-gray-700">Рост процентов и штрафов за <span x-text="months"></span> мес.</p>
          </div>
          <div class="bg-white rounded-lg p-4 shadow-sm">
            <div class="text-3xl font-bold text-red-600 mb-2">+7%</div>
            <p class="text-sm text-gray-700">Исполнительский сбор ФССП</p>
          </div>
          <div class="bg-white rounded-lg p-4 shadow-sm">
            <div class="text-3xl font-bold text-red-600 mb-2">Ограничения</div>
            <p class="text-sm text-gray-700">Арест счетов, запрет на выезд</p>
          </div>
        </div>
        <div class="mt-6 text-center">
          ...
          <p class="text-sm text-gray-600">
            Вместо экономии в <span x-text="formatNumber(discount) + ' ₽'"></span> с нами
          </p>
        </div>
      </div>
    </div>
