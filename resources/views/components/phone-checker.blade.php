...
<section class="py-16 md:py-24 text-slate-900 relative bg-white border-b border-slate-200 swiss-pattern-diagonal"
  x-data="phoneChecker()">
  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
    <div class="max-w-4xl mx-auto">
      <div class="mb-12 text-center relative">
        ...
        <h2 class="text-3xl md:text-4xl font-bold mb-6 tracking-tight font-serif" style="color: #243468;">Проверьте
          номер телефона</h2>
        <p class="text-lg text-slate-700 max-w-3xl mx-auto font-light">Если к вам поступают сообщения или звонки с
          угрозами или оскорблениями от имени {{ $legalName }}, вы можете проверить официальные номера телефонов,
          которые зарегистрированы на {{ $legalName }} и используются для взаимодействия с должниками по вопросам
          взыскания просроченной задолженности.</p>
      </div>
      ...
      <div class="border border-slate-200 rounded-none bg-stone-50/50 overflow-hidden">
        <div class="p-8 border-b border-slate-200">
          ...
          <form x-on:submit.prevent="search()" class="space-y-4">
            <div class="flex flex-col sm:flex-row gap-4">
              <div class="flex-1">
                <input type="tel" x-model="searchQuery" placeholder="+7 (800) 500-29-01 или 88005002901"
        ...
        <div class="p-8">
          <div x-show="result === 'found'" x-transition
            class="border-2 border-green-200 rounded-none bg-green-50/50 p-6">
            <div class="flex items-start gap-4">
              <div
                class="flex-shrink-0 h-12 w-12 rounded-none border-2 border-green-600 flex items-center justify-center bg-white">
              ...
              <div class="flex-1">
                <div class="flex items-start justify-between mb-4">
                  <div>
                    <h3 class="text-xl font-bold font-serif text-slate-900 mb-1" style="color: #243468;"
                      x-text="phone.phone_number"></h3>
                ...
                <div class="mt-4 p-4 bg-white border border-slate-200 rounded-none">
                  <p class="text-sm text-slate-700 font-light">✓ Этот номер телефона действительно принадлежит
                    {{ $legalName }} и используется для официального взаимодействия с должниками по вопросам взыскания
            ...
            <div class="mt-6">
              <button type="button" x-on:click="reset()"
                class="w-full py-3 border-2 border-slate-900 text-slate-900 rounded-none uppercase tracking-wider hover:bg-slate-50 transition-colors font-bold">Проверить
                другой номер</button>
            </div>
          </div>

          <div x-show="result === 'notfound'" x-transition
            class="border-2 border-red-200 rounded-none bg-red-50/50 p-6">
            <div class="flex items-start gap-4">
              <div
                class="flex-shrink-0 h-12 w-12 rounded-none border-2 border-red-600 flex items-center justify-center bg-white">
                <svg class="h-6 w-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </div>
              <div class="flex-1">
                <h3 class="text-lg font-bold font-serif text-slate-900 mb-2" style="color: #243468;">Номер не найден
                </h3>
                <p class="text-sm text-slate-700 mb-4 font-light">Этот номер телефона не зарегистрирован в нашей базе
                  официальных номеров {{ $legalName }}. Это может быть признаком мошенничества.</p>
                <div class="p-4 bg-white border border-slate-200 rounded-none mb-4">
                  ...
                  <ul class="text-sm text-slate-600 space-y-2 list-none">
                    ...
                    <li class="flex items-start gap-2">
                      <span class="text-slate-900 font-bold">•</span>
                      <span>Свяжитесь с нами по официальным каналам: <a
                          href="tel:{{ preg_replace('/\D/', '', $settings->phone ?? '88005002901') }}"
                          class="font-bold hover:opacity-80 transition-opacity"
                          style="color: #243468;">{{ $settings->phone ?? '8 800 500 29 01' }}</a></span>
                    </li>
                    <li class="flex items-start gap-2">
                      <span class="text-slate-900 font-bold">•</span>
                ...
                <button type="button" x-on:click="reset()"
                  class="w-full py-3 border-2 border-slate-900 text-slate-900 rounded-none uppercase tracking-wider hover:bg-slate-50 transition-colors font-bold">Попробовать
                  снова</button>
              </div>
            </div>
          </div>
        ...
        <div class="p-8 pt-0 border-t border-slate-200">
          <div class="p-6 bg-slate-50 border border-slate-200 rounded-none">
            <p class="text-sm font-bold uppercase tracking-wider text-slate-900 mb-3">Важно: Наши сотрудники никогда не
              будут:</p>
