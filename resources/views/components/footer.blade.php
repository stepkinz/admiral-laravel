  ...
  <div class="container mx-auto px-4 sm:px-6 lg:px-12 py-12">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div class="space-y-3">
        ...
        @if($footerInn)
        <p class="text-sm">ИНН: {{ $footerInn }}</p>@endif
        @if($footerOgrn)
        <p class="text-sm">ОГРН: {{ $footerOgrn }}</p>@endif
        @if($footerInn)
        <p class="text-sm">Реестр ФССП: № {{ $footerInn }}</p>@endif
      ...
      <div class="space-y-3">
        ...
        @if($footerAddress)
        <p class="text-sm">Адрес: {{ $footerAddress }}</p>@endif
      </div>
      ...
      <div class="space-y-3">
        ...
        <p class="text-sm leading-relaxed">
          «ПКО Адмирал» специализируется <strong>только</strong> на законных и реалистичных методах урегулирования (по
          ФЗ-230): фиксации долга и реструктуризации.
        </p>
      </div>
    </div>
  </div>
...
<footer class="bg-gray-900 text-gray-400 border-t border-gray-700">
  <div class="container mx-auto px-4 sm:px-6 lg:px-12 py-4">
    <div class="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 text-xs">
      <a href="{{ route('legal.privacy') }}" class="hover:text-white hover:underline transition-colors">Политика
        конфиденциальности</a>
      <span class="hidden sm:inline">•</span>
      <a href="{{ route('legal.terms') }}" class="hover:text-white hover:underline transition-colors">Пользовательское
        соглашение</a>
      <span class="hidden sm:inline">•</span>
      <a href="{{ route('legal.personal-data') }}" class="hover:text-white hover:underline transition-colors">Согласие
