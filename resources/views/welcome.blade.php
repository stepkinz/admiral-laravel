<x-layouts.app title="Адмирал — Помощь должникам">
  ...
  <div class="min-h-screen bg-gradient-to-b from-blue-50 to-white"
    x-data="{ showModal: {{ $openLeadModalOnLoad ? 'true' : 'false' }} }" @open-lead-modal.window="showModal = true">
    <x-top-bar />
    <x-header />
    ...
    <main class="min-h-screen">
      ...
      @if(isset($settings) && filled($settings->seo_text))
        <section class="container mx-auto px-4 py-8 max-w-site text-slate-600 text-sm prose prose-slate max-w-none"
          aria-label="Дополнительная информация для поисковых систем">
